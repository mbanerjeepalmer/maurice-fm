<script>
	import Title from '$lib/Title.svelte';
</script>

<title>Maurice Banerjee Palmer</title>
<h2>What I've learnt so far</h2>
<p>
	A collection of how-to guides based on what I've learnt so far as a product person. It's a work in
	progress.
</p>
<ul>
	<li><a href="./hacketisms">Hacketisms: Catechisms for a hackathon</a></li>
	<li><a href="./rube">A Rube Golderbergian Boilerplate for Claude</a></li>
	<li>
		<a href="./passable-cv">Write a passable CV</a>
	</li>
	<li>
		<a href="./money-transfer-companies">International money transfer companies</a>
	</li>
	<li><a href="./make-better-slides">Make better slides</a></li>
	<li><a href="./presenting-your-slides">Presenting your slides</a></li>
	<li><a href="./learn-to-code">Learn to code in 15 minutes</a></li>
	<li><a href="./useful-user-research">Useful user research</a></li>
</ul>
<h2>Other writing</h2>
<ul>
	<li><a href="./finest-tv">The final week of the finest television</a></li>
	<li>
		<a href="https://digitalfrontier.com/articles/rss-dead-protocols-internet-llm" target="_blank"
			>RSS is dead. Long live RSS!</a
		>
	</li>
	<li>
		<a href="https://sotaletters.substack.com/p/rockets" target="_blank"
			>Rockets, Culture and Progress</a
		>
	</li>
	<li>
		<a href="https://www.zacusca.net/blog/feedly-unusable" target="_blank"
			>Why is Feedly pushing unusable AI?</a
		>
	</li>
</ul>
<h2>What I've done</h2>
<h3>Consulting (now)</h3>
<ul>
	<li>
		AI for a 230-year-old auction house. Trained agentic engineering, shipped new products at 20x
		normal pace.
	</li>
	<li>User research training for a VC-backed proptech startup.</li>
</ul>
<h3><a href="https://bnqt.app/" target="_blank" rel="noopener noreferrer">Banquet</a></h3>
<p>
	<a href="https://bnqt.app/" target="_blank" rel="noopener noreferrer">Banquet</a>: Solo founded AI
	media startup building products to make the best use of your attention.
</p>
<p>
	I decided to stop after two years. I'll write a post-mortem at some point. For now you can read an
	early update <a href="./banquet-wave">here</a>). If you're working on something similar and want
	to learn about my experiences please drop me a line.
</p>

<h4><a href="https://anatole.fyi" target="_blank" rel="noopener noreferrer">Anatole</a></h4>
<p>A Slack bot that keeps you on top of everything you need to know for work.</p>
<h4><a href="https://printstack.net" target="_blank" rel="noopener noreferrer">Printstack</a></h4>
<p>Turn a Substack into a book.</p>
<h4><a href="https://vol.best" target="_blank" rel="noopener noreferrer">vol</a></h4>
<p>
	Search for podcasts using your ambitions instead of keywords. Built solo at Meta's Llama
	hackathon. See <a href="https://github.com/mbanerjeepalmer/vol-web">repo</a>
	and
	<a
		href="https://www.producthunt.com/products/vol/launches/vol-2"
		target="_blank"
		rel="noopener noreferrer"
		>Product Hunt #9 of the day</a
	>.
</p>
<h3>Filmmaking</h3>
<p>
	Helped produce and shoot
	<a
		href="https://filmsandfestivals.britishcouncil.org/projects/elephant-families"
		target="_blank"
		rel="noopener noreferrer"
		>Elephant Families</a
	>, a short documentary about the Latino community in South London. The film was selected for the
	<a
		href="https://whatson.bfi.org.uk/lff/Online/default.asp?BOparam::WScontent::loadArticle::permalink=discovering-home-lff25&BOparam::WScontent::loadArticle::context_id="
		target="_blank"
		rel="noopener noreferrer"
		>BFI London Film Festival</a
	>
	and
	<a
		href="https://boxoffice.hotdocs.ca/websales/pages/info.aspx?evtinfo=608269~cd63fa6a-1569-492d-89ae-a73ea714567e&epguid=f3bf8433-2ddd-4eb0-a2b5-e241bcf1021b&mdy=5/3/2026&"
		target="_blank"
		rel="noopener noreferrer"
		>Hot Docs</a
	>
	in Toronto.
</p>
<h3>
	<a
		href="https://www.linkedin.com/in/mbanerjeepalmer/details/experience/"
		target="_blank"
		rel="noopener noreferrer"
	>
		Seven years in product management</a
	>
</h3>
<p>
	Mostly in FinTech. At some companies you might have heard of, like Amex. Backed by some investors
	you might have heard of, like Andreesen Horowitz.
</p>
<h2>Find me</h2>
<p>If anything above interests you, I'm always happy to chat.</p>
<p>
	<a href="https://github.com/mbanerjeepalmer" target="_blank" rel="noopener noreferrer">GitHub</a>,
	<a href="https://linkedin.com/in/mbanerjeepalmer" target="_blank" rel="noopener noreferrer"
		>LinkedIn</a
	>, <a href="https://x.com/mbanerjeepalmer" target="_blank" rel="noopener noreferrer">X</a>,
	<a
		href="https://bsky.app/profile/mbanerjeepalmer.bsky.social"
		target="_blank"
		rel="noopener noreferrer"
		>Bluesky</a
	>, <a href="mailto:hello@maurice.fm" target="_blank" rel="noopener noreferrer">email</a>.
</p>

<style lang="postcss">
	a {
		@apply underline;
	}
	/* h2 {
		@apply mt-16;
	}

	h3 {
		@apply mt-12;
	} */
</style>
